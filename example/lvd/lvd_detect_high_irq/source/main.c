...
int main(void)
{
    ...
    // Clk_Enable(ClkXTL, FALSE);    // Disable XTL to use P15 as LVD out
      
    //配置测试IO口
    Gpio_InitIO(TEST_PORT, TEST_PIN, GpioDirOut);
    Gpio_SetIO(TEST_PORT, TEST_PIN, FALSE);

//    Gpio_SetFunc_LVDOUT_P25();//配置LVD输出口
...
//    Gpio_SetAnalog(2, 3, TRUE);
    ...
    stcLvdCfg.enInput = LvdInputP25;
//    stcLvdCfg.enInput = LvdInputP23;//配置LVD输入口
    //stcLvdCfg.enInput = LvdInputP03;
    stcLvdCfg.enThreshold = LvdTH3p0V;//配置LVD基准电压
    stcLvdCfg.bFilter = TRUE;
    stcLvdCfg.enFilterTime = LvdFilter29ms;
    stcLvdCfg.enIrqType = LvdIrqHigh;
